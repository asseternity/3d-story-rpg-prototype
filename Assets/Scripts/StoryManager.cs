...
public class StoryManager : MonoBehaviour
{
    ...
    public void StartSectionFromDialogue(IArticyObject aObject, string[] queue)
    {
        ...
        string[] nextThingParts = queue[0].Split('_');
        ...
        {
            Debug.LogError("Unknown next thing type: " + nextThingParts[0]);
        }
    }
    ...
    public void OnBranchesUpdated(IList<Branch> aBranches)
    {
        ...
        {
            // here we check what is in the queue
            if (nextThingType == "dialogue")
            {
                // look up the next dialogue by ID
